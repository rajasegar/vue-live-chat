<template>
  <form class="sc-user-input" v-bind:class="{ 'active' : inputActive}">
        <div
          role="button"
          tabIndex="0"
        ...
        <div class="sc-user-input--buttons">
          ...
          <div class="sc-user-input--button">
            <EmojiIcon v-if="showEmoji" v-on:new-emoji="sendEmojiMessage" />
          </div>
          <div class="sc-user-input--button">
            <SendIcon v-on:submit-message="handleSubmitButton" />
          </div>
        </div>
      </form>
</template>
...
export default {
  ...
  methods: {
    ...
    handleEnterButton: function(e) {
      e.preventDefault();
      this.sendTextMessage(e.target.textContent.trim());
      e.target.innerHTML = '';
    },
    
    handleSubmitButton: function() {
      this.sendTextMessage(this.message); 
      this.message = '';
    },
    ...
    sendTextMessage: function(text) {
      ...
      this.$emit('send-message', _message);
    ...
    readMessage: function(e) {
      this.message = e.target.textContent.trim();
    }
